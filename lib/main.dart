...
class Todo {
  ...
  //konstruktor
  Todo({required this.syssla, this.kryssbox = false, this.id = 'id'});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          actions: [
            PopupMenuButton(
                onSelected: (String value) => {
                      Provider.of<TodoList>(context, listen: false).setFilterBy(
                        value,
                      )
                    },
                itemBuilder: (context) => [
      ...
      body: Consumer<TodoList>(
        builder: (context, state, child) => _list(state.getFilteredList(
          state.list,
          state.filterBy,
        )),
      ),
